...
const Detail = () => {
  ...
  const handleAddToCart = async () => {
    ...
    let itemExist = cart.cartList.find((cartItem) => cartItem.id === item.id);

    if (itemExist) {
      if (itemExist.quantity + item.quantity > 8) {
        setAlert({
          type: "error",
          message: "Total quantity of this book in cart is not allowed over 8 ",
        });
                ...
                <div className="price-detail">
                  <div className="price">
                    {detail.discountPrice ? (
                      <>
                        <span
                          className={classNames("current", {
                            "fs-line-through": detail.discountPrice,
                          })}
                        >
                          {detail.price}$
                        </span>
                      ...
                      <span className="current">{detail.price}$</span>
                    )}
                  </div>
                  ...
                  <div className="quantity">
                    ...
                    <div className="control">
                      <span className="minus" onClick={handleDecreaseQuantity}>
                        -
                      </span>
                      <span className="number">{detail.quantity}</span>
                      <span className="plus" onClick={handleIncreaseQuantity}>
                        +
                      </span>
                    </div>
